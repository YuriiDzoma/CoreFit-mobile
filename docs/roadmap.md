...
- [ ] Program edit (needs real update semantics, not a port of web's broken create-duplicate behavior, and blocked on a live RLS gap — see `docs/decisions.md`), training history + workout logging (`exercise_drafts`/`exercise_logs`/`training_history` — mobile can now display the feed but still has no way to create entries), Global Programs/Complexes — all downstream of this foundation, not yet started
