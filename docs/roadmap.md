...
- [ ] Program edit (needs real update semantics, not a port of web's broken create-duplicate behavior), training history + workout logging (`exercise_drafts`/`exercise_logs`/`training_history`), delete, Global Programs/Complexes, and the future Dashboard (Home tab) — all downstream of this foundation, not yet started
